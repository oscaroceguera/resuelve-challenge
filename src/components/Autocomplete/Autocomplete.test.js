...
describe('AutomComplete', () => {
  ...
  it('[render without crashing]', () => {
    const data = []
    wrapper = shallow(<AutoComplete data={data} handleItemSelected={f => f} />)
    expect(shallowToJson(wrapper)).toMatchSnapshot()
  })

  it('[render with all props]', () => {
    const data = []
    wrapper = shallow(<AutoComplete data={data} handleItemSelected={f => f} placeholder={'PLACEHOLDER'} width={'Large'} required />)
    expect(shallowToJson(wrapper)).toMatchSnapshot()
  })
